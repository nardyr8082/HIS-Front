...
import {
  LanguageInfoService
} from 'src/app/backend/services/language-info/language-info.service';
import {
  ImagePickerConf
...
@Component({
  selector: 'app-dialog-add-edit-language-info',
  templateUrl: './dialog-add-edit-language-info.component.html',
  styleUrls: ['./dialog-add-edit-language-info.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class DialogAddEditLanguageInfoComponent implements OnInit, OnDestroy {
  isSaving = false;
  isEditing = false;
  loggedInUser: any;
  form: FormGroup;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef < DialogAddEditLanguageInfoComponent > ,
    ...
    this.selectedLanguageInfo = data.selectedLanguageInfo;
  ...
  createForm(): void {
    this.form = this.fb.group({
      MailTemplateId: [this.selectedLanguageInfo?.MailTemplate?.id, []],
      values: [this.selectedLanguageInfo?.values, []],
  });
...
onSave(): void {
  ...
  if (!this.isEditing) {
    ...
    let dataOutput = {
      id: this.selectedLanguageInfo.id
    };
    for (let key in data) {
      if (!this.utilsService.isObjectEquals(this.selectedLanguageInfo[key], data[key])) {
        dataOutput[key] = data[key];
      }
    }
    this.languageInfoService.editLanguageInfo(dataOutput).subscribe(
      () => {
