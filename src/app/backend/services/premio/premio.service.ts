...
export class PremioService {
  url = environment.apiUrl + 'premio';
  urlId = environment.apiUrl + 'premio/:id';
  httpOptions = {};

  constructor(private httpClient: HttpClient) {}

  createPremio(body: any): Observable < any > {
  ...
  getAllPremios(query ? : IPagination, params ? : any) {
    ...
    if (params) {
      ...
      if (params.items != undefined) {
        httpParams = httpParams.set(
          'filter[$and][items][$like]', '%' + params.items.toString() + '%');
      }
    }
  ...
  getPremio(data: any): Observable < any > {
