...
export class BoxstockFormComponent implements OnInit, OnDestroy {
  ...
  buildForm() {
    this.boxstockForm = new FormGroup({
      ...
      nro: new FormControl(this.data?.boxstock?.nro ? this.data?.boxstock.nro : null, [Validators.required, (Validators.maxLength(18))], MyValidation.validateFieldNumber(this.boxstockService)),
