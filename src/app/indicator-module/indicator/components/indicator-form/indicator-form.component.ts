...
export class IndicatorFormComponent implements OnInit, OnDestroy {
  ...
  buildForm() {
    this.indicatorForm = new FormGroup({
      nombre: new FormControl(this.data.indicator ? this.data.indicator.nombre : '', [Validators.required,Validators.pattern('^[0-9]+([,][0-9]+)?$')]),
      especificidad: new FormControl(this.data.indicator ? this.data.indicator.especificidad : ''),
      variables: new FormControl(this.data.indicator ? this.data.indicator.variables : ''),
      objetivo: new FormControl(this.data.indicator ? this.data.indicator.objetivo : ''),
      formula_de_calculo: new FormControl(this.data.indicator ? this.data.indicator.formula_de_calculo : ''),
      interpretacion: new FormControl(this.data.indicator ? this.data.indicator.interpretacion : ''),
      desglose_especifico: new FormControl(this.data.indicator ? this.data.indicator.desglose_especifico : ''),
      metodo_medicion: new FormControl(this.data.indicator ? this.data.indicator.metodo_medicion : ''),
      fuente_indicador: new FormControl(this.data.indicator ? this.data.indicator.fuente_indicador : ''),
