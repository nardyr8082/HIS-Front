...
export class HealthUnitFormComponent implements OnInit, OnDestroy {
  ...
  buildForm() {
    this.healthUnitForm = new FormGroup({
      ...
      telefono_movil: new FormControl(this.data.healthUnit ? this.data.healthUnit.telefono_movil : '+(240)', [ Validators.pattern('^[+]*[(]{1,2}[0-9]{1,4}[)][-\\s\\.]?[0-9]{3}[-\\s\\.]?[0-9]{4,6}$')]),
      telefono_fijo: new FormControl(this.data.healthUnit ? this.data.healthUnit.telefono_fijo : '+(240)', [ Validators.pattern('^[+]*[(]{1,2}[0-9]{1,4}[)][-\\s\\.]?[0-9]{3}[-\\s\\.]?[0-9]{4,6}$')]),
    });
  }
