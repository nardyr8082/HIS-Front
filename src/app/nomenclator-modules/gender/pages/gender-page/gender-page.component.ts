...
export class GenderPageComponent implements OnInit, OnDestroy {
  ...
  getGenders(filters = this.filters, sortColumn = 'id', sortDirection = 'desc', page = this.page, pageSize = this.pageSize) {
    this.loading = true;
      ...
      .pipe(
        switchMap((gender: Gender) =>
          this.genderService.createGender(gender).pipe(
            ...
            tap((success) => {
              if (success) {
                this.getGenders(this.filters, 'id', 'desc', this.page, this.pageSize);
