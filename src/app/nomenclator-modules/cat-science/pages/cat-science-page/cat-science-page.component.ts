...
export class CatSciencePageComponent implements OnInit, OnDestroy {
  ...
  getCatScience(filters = this.filters, sortColumn = 'id', sortDirection = 'desc', page = this.page, pageSize = this.pageSize) {
    this.loading = true;
      ...
      .pipe(
        switchMap((catScience: CatScience) =>
          this.catScienceService.createCatScience(catScience).pipe(
            ...
            tap((success) => {
              if (success) {
                this.getCatScience(this.filters, 'id', 'desc', this.page, this.pageSize);
